...
def print_scores(scores,method):
    print (method+'\n-----------------------------')
    print ("Most relevant : "+ str(option_values[scores.index(max(scores))]))
    print ("Least relevant : "+ str(option_values[scores.index(min(scores))]))
...
if __name__ == '__main__':
    while True:
        ...
        if response.status_code != 200 or response.json().get('error'):
            ...
            print_scores(normal,'Question search')
            withoption = scores_with_options(question,options)
            print_scores(withoption,'Question and options search')
